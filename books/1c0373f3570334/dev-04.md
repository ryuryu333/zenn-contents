...
# 1. この章でやること
この章では、`flake.nix` にて devShell 定義するために必要な Nix 言語文法や仕様を解説します。
...
# 2. 最小構成での解説
前章で提示した `flake.nix` には様々なテクニックが内包されているため、作成過程を完結に説明するのは難しいです。
...
それでは、`flake.nix` の記述を要素ごとに見ていきます。
...
今回の例だと、hello パッケージを devShell で利用します。
そのためには、hello のビルドレシピを用意する必要があります。
通常、パッケージのビルドレシピは Nixpkgs という GitHub レポジトリから取得します。
...
また、複数のパッケージを内包した環境をビルドするためのレシピのことも Derivation と表現します。

>大雑把な表現ですが、何らかを構築するための機能は大抵 Derivation を引数としますし、環境を定義するための関数は大抵 Derivation を返します。
...
# 3. 保守性を高めた書き方に変更する
前章で示した例と異なり、先ほどの `flake.nix` はあえて冗長、かつ、マジックナンバー上等といったスタイルで書いていました。
