...
# 1. この章でやること
この章では、**Homebrew で入れていたツールを home-manager に移行する**ことを目標にします。

具体例として、Homebrew 管理下の **Git と zsh** を home-manager に移行し、設定ファイルも合わせて管理します。

>段階的に進めるため、既存の Homebrew と **併用**します。
zsh 以外のシェルを使っている場合、ツール名や設定ファイルを自身のものに置き換えてください。
...
（`dotfiles` にて home-manager 以外も管理したいので、`.config/` 以外で管理しています。）
...
:::message
このままだと、すでにある `~/.gitconfig` と home-manager が配置しようとする `.gitconfig` が競合します。
...
# 4. シェル設定（zsh）を移行する
`~/.config/home-manager/zsh/.zshrc`、`~/.config/home-manager/zsh/.zprofile` も同様にします。
