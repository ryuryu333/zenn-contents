...
**上記以外のパッケージの場合、`Nixpkgs をピン留め` がおすすめ**。
**`nix-versions を利用` も有用、好みの方で良いと思います**。

前者は仕組みがシンプル、設定により何が起こるか把握しやすいです。
後者は利便性が高いが公式外の仕組みに依存し、かつ、内部挙動が把握しにくいです。

----

**Nixpkgs 未登録のバージョンを使いたいなら `overrideAttrs でパッケージ定義を上書き`**。
