...
ローカルで環境を作り終われば、そのまま CI で実行するだけという状態になるので楽でした。

備忘録を兼ねて、本記事ではどのように Nix を CI に用いているかを紹介します。
...
:::message
ローカルと CI で同じコマンドを実行すれば、同じ結果が得られる、というのが楽なポイントだと思いました。
...
marp-action 固有のお作法を覚えるのが面倒なので、Nix devShells を利用する方向性で組みました。
:::

作成したプロジェクトから public にしたく無いモノを除いたレポジトリを用意しました。
ただし、色々と機能を詰め込みすぎたので、本記事では「CI に Nix を使うことのみ」にフォーカスして、簡易化したコードを紹介します。
