...
# はじめに
GitHub Actions の CI/CD に Nix を利用する際、バイナリキャッシュを活用することでビルド時間を短縮可能です。
...
# キャッシュの保存先を作成
ログイン後、外面上部のメニューから Caches ページを開きます。
New Cache をクリックします。
...
:::message
自身のキャッシュを公開したくない場合、以下のツールも選択肢となるでしょう。
...
`Run cachix/cachix-action@v16` では、最初のビルドなので何もキャッシュは読み込まれていないようです。
...
つまり、ビルド時には以下の様に公式のバイナリキャッシュ（`cache.nixos.org`）を利用しています。
...
# Cachix が効果を発揮するケース
node_module を Nix でビルドする場合、キャッシュによりビルドが有意に高速化しました。
（2m 10s -> 51s : 60% 短縮）


対称的に、今回例示した `flake.nix` を用いたフローではキャッシュの効果を感じられませんでした。
（46s -> 41s : 10% 短縮）
