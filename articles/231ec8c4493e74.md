...
### 既存環境の確認・クリーンアップ
公式の Docs に従い、既存の Node.js があるかをチェック、存在するならがアンインストールします。
...
- `%ProgramFiles%\nodejs` が存在するかチェック、あるなら消去します

### インストーラーの実行
releases から `nvm-setup.exe` をダウンロードし、実行します。
...
（Claude Code における `CLAUDE.md`、Github Copilot における `copilot-instructions.md` です。）
