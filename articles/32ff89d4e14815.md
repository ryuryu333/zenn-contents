...
**本記事では、`textlint` と `textlint プラグイン` を Nix で管理することをゴールとして、その方法を解説します**。
...
- Node.js パッケージを Nix で管理したい方
  - pkgs に未登録のパッケージ含む
- ホスト/プラグイン型（単独では機能しない）パッケージを使用したい方
...
**コマンドだけ知りたい方はクイックスタートだけ読めば大丈夫です**。

他のセッションは興味があればお読みください。
...
node2nix は npm の依存解決結果を Nix に落とし込みのため、textlint --preset ja-technical-writing README.md を実行すると失敗します。、ネットワーク不要・再現可能に Node.js ツールを配布/利用できるようにするツールです。用途に応じて大きく2通りの使い方があります。

A. node-packages.json に使いたいパッケージのリストを書く
B. 既存の package.json と package-lock.json（または npm-shrinkwrap.json）を元に生成する

前者は単体 CLI のピン止めに便利、単体/少数の CLI（例: prettier, eslint, typescript）をサッと固定したい。


後者は複数ツールやプラグイン解決を lockfile に任せたいときに便利です。eslint + プラグイン群など、peerDependencies を含む複数ツールを lockfile に沿って厳密に再現したい
既存の Node プロジェクト（package.json/package-lock.json）をそのまま Nix に固定したい

単体 CLI をサクッと使いたい: A（node-packages.json）
