---
title: "社用 PC で github 接続時に SSL エラーが起きた際の対処法"
emoji: "🍀"
...
# 概要
社用 PC にインストールされているセキュリティソフトによっては、SSL インスペクション機能により、git fetch 等のコマンドを実行時に下記エラーが発生します。
...
>クライアントとサーバーの間のSSL暗号化されたインターネット通信を捕捉し、検査することです。インターネット トラフィックの大部分はSSL暗号化されていますが、悪意のあるコンテンツが含まれている場合もあるため、SSLトラフィックの検査は極めて重要になっています。

git 等の一部のアプリケーションでは、通信に Zscaler が割り込むことによって、証明書の検証が上手くいかずエラーとなります。
...
Zscaler の SSL インスペクションが有効となっている場合、ルート証明書（depth=2）が取得できずにエラーとなります。
...
Zscaler 公式より、git 等の一部のアプリケーションでこの様な問題が起こると記載があります。
...
# おわりに
安直に `git config --global http.sslVerify false` としてはいけない。
何が原因でエラーが発生しているかを精査することで、適した設定を行いセキュリティ維持したままでエラーを解消できます。
